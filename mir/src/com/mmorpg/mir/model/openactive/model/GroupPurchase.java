...
import com.mmorpg.mir.model.openactive.packet.SM_GroupPurchase_Attend_Satisfy;
import com.mmorpg.mir.model.openactive.packet.SM_GroupPurchase_Gold_Change;
...
public class GroupPurchase {
	...
	public static GroupPurchase valueOf() {
		GroupPurchase result = new GroupPurchase();
	...
	public void addGold(Player player, long amount) {
		...
		for (GroupPurchaseResource resource : OpenActiveConfig.getInstance().groupPurchaseStorage.getAll()) {
			...
			if (!attendGroupPurchase && resource.getAttendConditions().verify(player.getObjectId())) {
				...
				if (attendAmountVerify) {
					sendAttendPlayerPacket(
							resource.getId(),
							SM_GroupPurchase_Attend_Satisfy.valueOf(resource.getId(),
									groupPurchasePlayers.get(resource.getId()).size()));
				}
			}
		}
		PacketSendUtility.sendPacket(player, SM_GroupPurchase_Gold_Change.valueOf(this.goldAmount));
	}

	@JsonIgnore
	private void sendAttendPlayerPacket(String resoureId, Object packet) {
		for (Long attendPlayerId : ServerState.getInstance().getGroupPurchasePlayers().get(resoureId)) {
			GroupPurchase groupPurchase = ServerState.getInstance().getPlayerGroupPurchases().get(attendPlayerId);
