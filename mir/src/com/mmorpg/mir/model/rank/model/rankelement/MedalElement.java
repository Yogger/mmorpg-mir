...
public class MedalElement extends RankRow {
	...
	public static MedalElement valueOf(Player player) {
		MedalElement e = new MedalElement();
		...
		e.level = player.getCombatSpiritStorage().getCombatSpiritCollection()
				.get(CombatSpiritType.MEDAL.getValue()).getCombatResourceId();
	...
	public void changeByEvent(IEvent event) {
		if (event instanceof CombatSpiritUpEvent) {
			CombatSpiritUpEvent up = (CombatSpiritUpEvent) event;
			level = up.getResourceId();
