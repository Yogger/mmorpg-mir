...
public class CountryNpcFlagAndDachenContoller extends CountryNpcController {
	...
	public void onAttack(Creature creature, int skillId, int damage) {
		super.onAttack(creature, skillId, damage);
	...
	public void onFightOff() {
		super.onFightOff();
		countryDamage.clear();
		boolean isFlag = ConfigValueManager.getInstance().COUNTRY_DIP_NPC_FLAG.getValue().values()
				.contains(getOwner().getSpawnKey());
		String start = isFlag ? ConfigValueManager.getInstance().COUNTRY_SAFE_START_CRON_FLAG.getValue()
				: ConfigValueManager.getInstance().COUNTRY_SAFE_START_CRON.getValue();
		String end = isFlag ? ConfigValueManager.getInstance().COUNTRY_SAFE_END_CRON_FLAG.getValue()
		...
		if (conditions.verify(new Object(), false)) {
	...
	protected synchronized void broadDamage() {
		final boolean isFlag = ConfigValueManager.getInstance().COUNTRY_DIP_NPC_FLAG.getValue().values()
				.contains(getOwner().getSpawnKey());
					...
					for (VisibleObject vi : getOwner().getKnownList()) {
						if (vi instanceof Player) {
							...
							PacketSendUtility.sendPacket((Player) vi, SM_CountryNpc_Fight_Rank.valueOf(tempDamageVOs,
									getOwner().getAggroList().getCountryDamage(), maxHp, isFlag ? 0 : 1, getOwner().getCountryValue()));
						}
					}
				}
			}, 2000, 2000);
