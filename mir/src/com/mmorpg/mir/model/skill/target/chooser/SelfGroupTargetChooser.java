...
@Component
public class SelfGroupTargetChooser extends AbstractTargetChooser {

	@Override
	public TargetType getTargtType() {
		return TargetType.SELF_RANGE_GROUP;
	}
	...
	protected boolean xyTargetSelect(Skill skill, Creature target) {
		if (skill.getFirstTarget() instanceof Player) {
			Player player = (Player) skill.getFirstTarget();
			if (player.isInGroup() && player.getPlayerGroup().getMembers().contains(target)) {
