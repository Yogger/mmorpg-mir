...
public class HpstoreHealHotEffect extends EffectTemplate {
	...
	public void endEffect(Effect effect) {
		...
		effected.getEffectController().unsetAbnormal(EffectId.HPRESTORE.getEffectId());
	}

	@Override
	public void onPeriodicAction(Effect effect) {
		super.onPeriodicAction(effect);
		if (!effect.getEffected().getLifeStats().isFullyRestoredHp()
				&& !effect.getEffected().getLifeStats().isAlreadyDead()) {
			long increase = effect.getEffected().getLifeStats().getMaxHp()
					- effect.getEffected().getLifeStats().getCurrentHp();
			increase = Math.min(Math.min(effect.getReserved2(), effect.getReserved3()), increase);
			effect.getEffected().getLifeStats().increaseHp(increase);
	...
	public void startEffect(final Effect effect) {
		...
		effected.getEffectController().setAbnormal(EffectId.HPRESTORE.getEffectId());
