...
public class PlayerService {
	...
	public PlayerVO login(TSession session, String account, String op, String server, String sign, int isAdult,
		...
		if (operatorManager.isBan(player.getObjectId()) && !session.hasAttribute("ATT_ALLOW")) {
		...
		if ((operatorManager.getSuperVip(server).isInit() || Debug.debug)
				&& operatorManager.getSuperVip(server).isOpen()) {
			vo.setSuperVipVO(operatorManager.getSuperVip(server).createVO(
					player.getVip().isSuperVip(operatorManager.getSuperVip(server))));
			vo.getSuperVipVO().setFirstChargeTime(player.getVip().firstChargeTime());
			vo.getSuperVipVO().setTotalCharge(player.getVip().totalCharge());
		}
		return vo;
	}
	...
	public PlayerVO createPlayer(TSession session, String account, String op, String server, String sign,
			...
			String time, int opVip, boolean forbidSelectCountryAccount, String refer) {
		if (role == Role.SORCERER.value()) {
			throw new ManagedException(ManagedErrorCode.VALIDATE_FAILURE);
		}
		// if (forbidSelectCountryAccount) {
		// if (country == playerManager.forbidSelectCountry()) {
		// throw new ManagedException(ManagedErrorCode.FORBID_COUNTRY);
		// }
		// }
		if (!Debug.debug) {
			if (Long.valueOf(time) + DateUtils.MILLIS_PER_MINUTE * 30 < System.currentTimeMillis()) {
		...
		String checkName = playerName.substring(("s" + server).length() + 1);
		// playerName命名是否合法
		if (DirtyWordsManager.getInstance().containsWords(checkName, WordsType.ROLEWORDS)) {
		...
		// TODO 根据不同平台发放奖励
		vipManager.rebate(player);
		commonActivityManager.rewardSpecialServer(player);
		EventBusManager.getInstance().syncSubmit(CreateEvent.valueOf(player.getPlayerEnt().getId(), op));
	...
	public static void main(String[] args) {
		String name = "s50037.dd";
		System.out.println(name.substring("s50037".length() + 1));
	...
	private void doLogin(TSession session, final Player player, int isAdult, int loginType, int opVip) {
		...
		player.getPlayerEnt().setLastLogin(now);
		...
		LogManager.loginLog(player, System.currentTimeMillis(), loginType, session.getInetIp(), player.getPlayerEnt()
				.getCountry());
		playerManager.doPlayerPositionFix(player);
		...
		// 第一次登陆修复爬塔熟据 (只处理一次)
		CopyManager.getInstance().ladderCopyFix(player);
	...
	public PlayerExpUpdate suicideTurn(Player player) {
		return playerManager.suicideTurn(player);
	}
		...
		if (player != null) {
			try {
				String message = MessageFormatter.format(
						"playerId[{}] session[{}]登出事件！",
						new Object[] { player.getName(),
								player.getSession() == null ? null : player.getSession().getId() }).getMessage();
				if (logger.isDebugEnabled()) {
					logger.debug(message);
				}
				player.getMoveController().stop();
				World.getInstance().despawn(player);
				CountryCopyManager.getInstance().removeTechCopySpeicialBuff(player);
				...
				CountryManager.getInstance().logoutTraitorPlayrFix(player);
				// 防沉迷
				...
				player.getEquipmentStorage().stopPetTask(player);
				// 删掉美人
				player.getBeautyGirlPool().doRest();
				...
				player.getEffectController().unsetAbnormal(EffectId.GM_NICKNAME, false);
