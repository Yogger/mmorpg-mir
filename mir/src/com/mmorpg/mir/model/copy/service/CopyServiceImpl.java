...
public class CopyServiceImpl implements CopyService {
		...
		if (resource.getType() == CopyType.EXP) {
			copyInfo.setCopyDestoryController(new CopyDestoryController() {
				...
				public boolean destory() {
					long destoryTime = worldMapInstance.getCreateTime() + resource.getCopyDestoryTime()
	...
	public void reward(String id, Player player) {
		...
		if (resource.getType() == CopyType.MINGJIANG) {
			...
			QuestResource questResource = QuestManager.getInstance().questResources.get(
					String.valueOf(curHorseEquipQuest), true);
	...
	public void encourge(Player player, boolean gold) {
		...
		if (success) {
			...
			PacketSendUtility.sendPacket(
					player,
					SM_Copy_Encourage.valueOf(copyInfo.getEncourgeCount(), copyInfo.getHpCount(),
							copyInfo.getDamageCount(), false));
		}
	}
	...
	public void horseCopyReset(Player player, String id) {
		...
		if (horseEquipMaxQuestHis == null
				|| horseEquipMaxQuestHis < Integer
		...
		Reward reward = rewardManager.creatReward(player, rewardId, null);
		List<String> rewardIds = chooserManager.chooseValueByRequire(player, copyManager.HORSEEQUIP_BOSS_DROP
				.getValue().get(id));
