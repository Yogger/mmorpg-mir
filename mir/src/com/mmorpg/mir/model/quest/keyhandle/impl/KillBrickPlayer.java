...
@Component
public class KillBrickPlayer extends QuestKeyHandle {

	@Override
	public KeyType getType() {
		return KeyType.KILL_BRICK_PLAYER;
	}

	// 所接收的事件
	@ReceiverAnno
	public void completeEvent(PlayerDieEvent event) {
		if (!event.isKilledByPlayer()) {
			return;
		}
		Player killed = playerManager.getPlayer(event.getOwner());
		if (killed.getTempleHistory().getCurrentBrick() == null) {
			return;
		}
		Player player = playerManager.getPlayer(event.getAttackerId());
