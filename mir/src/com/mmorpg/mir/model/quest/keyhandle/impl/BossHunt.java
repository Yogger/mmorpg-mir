...
@Component
public class BossHunt extends QuestKeyHandle {
	
	public static final String BOSSID = "BOSSID";

	@Override
	public KeyType getType() {
		return KeyType.BOSS_HUNT;
	}
			...
			for (QuestKey key : quest.getKeys()) {
				...
				if (key.getType() == this.getType()) {
					if (key.getParms() != null) {
						String bossId = (String) key.getParms().get(BOSSID);
						boolean checkMonster = (bossId != null ? bossId.equals(event.getBossId()) : true);
	...
	public QuestKey create(QuestKeyResource resource, long now, Player player) {
		...
		if (resource.getParms() != null) {
			...
			key.getParms().put(BOSSID, resource.getParms().get(BOSSID));
		}
		return key;
	}
