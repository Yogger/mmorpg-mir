...
@Component
public class FeteAll extends QuestKeyHandle {

	@Override
	public KeyType getType() {
		return KeyType.FETE_ALL;
	}

	// 所接收的事件
	@ReceiverAnno
	public void rescueEvent(CountrySacrificeEvent event) {
		...
		for (Quest quest : quests) {
			for (QuestKey key : quest.getKeys()) {
				if (key.getType() == this.getType()) {
					int count = player.getWelfare().getHistoryFeteCount();
	...
	public QuestKey create(QuestKeyResource resource, long now, Player player) {
		...
		key.setValue(player.getWelfare().getHistoryFeteCount());
