...
@Component
public class RescueAll extends QuestKeyHandle {

	@Override
	public KeyType getType() {
		return KeyType.RESCUE_ALL;
	}
		...
		for (Quest quest : quests) {
			for (QuestKey key : quest.getKeys()) {
				if (key.getType() == this.getType()) {
					int count = player.getRescue().getTotalCompleteCount();
	...
	public QuestKey create(QuestKeyResource resource, long now, Player player) {
		...
		key.setValue(player.getRescue().getTotalCompleteCount());
